<%@page import="com.itwillbs.sell.action.SellPantsList"%>
...
<!-- 게시판 내용 여기부터 넣으세요  -->
	...
	ArrayList<SellDTO> sellPantsList = (ArrayList<SellDTO>)request.getAttribute("sellPantsList");
...
<table>
	<tr> <!--  테이블................1칸 -->
	<%
	for(int i=0; i<sellPantsList.size();i++){
		SellDTO dto = sellPantsList.get(i);
	
	%>
		<td>
			<table class="item-table">
				<tr>
					<td colspan="2" class="S_img"><a href="SellPantsDetails.sell?S_num=<%=dto.getS_num()%>" >
					<img src="img/sell/<%=dto.getS_img() %>" width=300px height=300px class="goodsImg"></a></td>
				</tr>
				<tr>
					<td colspan="2" class="S_title" > <a href="SellPantsDetails.sell?S_num=<%=dto.getS_num()%>" > <%=dto.getS_title()%></a></td> <!-- 제목 -->
				</tr>
				<tr>
					<td class="price"><%=dto.getS_price()%>원</td> <td align="right" class="like_id"><input type="image" name="button"  class="heart" src="sell/heart.png" onclick="location.href='LikePro.like?S_num=<%=dto.getS_num() %>'"></tr>
				<%-- <tr>
...
<!-- 게시판 내용 여기 넘어가면 안됨.  -->							
				...
				for(int i=startPage;i<=endPage;i++){
					%>
					<a href="Pants.sell?pageNum=<%=i%>"><%=i%></a>
					<%
				}
